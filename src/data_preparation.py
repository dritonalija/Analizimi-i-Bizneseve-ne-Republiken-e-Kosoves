...
def normalize_status(df):
    ...
    df['Data e mbylljes'] = pd.to_datetime(df['Statusi'].str.extract(r'(\d{2}/\d{2}/\d{4})')[0], format='%d/%m/%Y', errors='coerce')
    df['Statusi'] = df['Statusi'].replace({r"(?i)pasiv.*": "Pasiv", r"(?i)anuluar nga sistemi": "Shuar"}, regex=True)
    return df
...
def detect_gender_gender_guesser(name):
    ...
    return "Mashkull" if guess in ["male", "mostly_male"] else "Femër" if guess in ["female", "mostly_female"] else "Panjohur"
...
def update_gender_column(df):
    ...
    df["Gjinia e pronarit"] = df.apply(
        lambda row: ", ".join(detect_gender_gender_guesser(name.strip()) for name in str(row["Pronarë"]).split(","))
        if "Panjohur" in str(row["Gjinia e pronarit"]) else row["Gjinia e pronarit"], axis=1)
    return df
...
def count_genders(df):
    ...
    df["Pronarë Mashkull"] = df["Gjinia e pronarit"].apply(lambda x: [g.strip() for g in str(x).split(",")].count("Mashkull"))
    df["Pronarë Femër"] = df["Gjinia e pronarit"].apply(lambda x: [g.strip() for g in str(x).split(",")].count("Femër"))
    return df
...
def extract_registry_number(df):
    ...
    df["Uid"] = df["Linku në ARBK"].str.split(",").str[-1].apply(lambda x: int(x) if pd.notnull(x) and x.isdigit() else None).astype("Int64")
    return df
...
def fill_business_type(df):
    ...
    type_map = {
        "B.I.": "Biznes individual", "SH.P.K.": "Shoqëri me përgjegjësi të kufizuara", "L.L.C.": "Shoqëri me përgjegjësi të kufizuara",
        "K.B.": "Kooperativa Bujqësore", "N.SH.": "Ndërmarrje shoqërore", "O.K.": "Ortakëri e kufizuar",
        "O.P.": "Ortakëri e përgjithshme", "SH.A.": "Shoqëri aksionare", "Z.P.K.": "Zyra e Përfaqësisë në Kosovë"
    }
    df["Tipi i biznesit"] = df.apply(lambda row: next((v for k, v in type_map.items() if row["Emri i biznesit"].endswith(k)), row["Tipi i biznesit"]), axis=1)
    return df
