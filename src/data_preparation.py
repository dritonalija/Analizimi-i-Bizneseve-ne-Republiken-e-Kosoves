...
def binarize_gender(df):
    ...
    df["Gjinia_Binar"] = df["Gjinia e pronarit"].apply(lambda x: 1 if x == "Mashkull" else 0)
    return df
...
def sample_data(df, fraction=0.1, random_state=42):
...
def preprocess_data(file_path, output_path):
    ...
    df = dimension_reduction(df, ["Numri fiskal", "Personat e autorizuar"])
    ...
    df = sample_data(df, fraction=0.1)
...
if __name__ == "__main__":
    ...
    output_file_path = "../data/processed/preprocessed_data-2023.csv"  # Adjusted path for output file
